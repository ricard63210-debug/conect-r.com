...
export default function TvMenuBoards() {
  ...
  const handleOpenChat = () => {
    window.dispatchEvent(
      new CustomEvent("conectr:open-chat", {
        detail: {
          greeting: T.tvMenuBoards.chatGreeting,
          lang,
          userMessage: T.tvMenuBoards.chatUserMessage,
        },
      }),
    );
  };

  const features = [RefreshCw, MessageCircle, Layout, Tv, Cloud, Monitor].map((icon, idx) => ({
    icon,
    title: T.tvMenuBoards.features.list[idx].title,
    body: T.tvMenuBoards.features.list[idx].body,
  }));

  const whyUs = [Zap, Clock, BookOpen, Star].map((icon, idx) => ({
    icon,
    title: T.tvMenuBoards.whyUs.list[idx].title,
    body: T.tvMenuBoards.whyUs.list[idx].body,
  }));
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      ...
      <section className="relative pt-16 pb-20 sm:pt-24 sm:pb-32 overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 text-center">
          ...
          >
            ...
            >
              <span className="relative z-10">{T.tvMenuBoards.hero.cta}</span>
      ...
      <section className="relative border-t border-border bg-muted/30">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-20 sm:py-28">
          <div className="text-center mb-14">
            <div className="flex justify-center mb-6">
              <div className="px-4 py-1 rounded-full border border-orange-500/20 bg-orange-500/5 text-orange-500 text-[10px] sm:text-xs font-bold tracking-[0.2em] uppercase">
                {T.tvMenuBoards.pricing.pill}
              </div>
            </div>
            <h2 className="text-3xl sm:text-5xl font-serif italic mb-4">{T.tvMenuBoards.pricing.title}</h2>
            <p className="text-muted-foreground max-w-xl mx-auto font-light">
              {T.tvMenuBoards.pricing.subtitle}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl mx-auto">
            ...
            >
              <div className="text-xs font-black tracking-[0.2em] text-orange-500 uppercase mb-4">{T.tvMenuBoards.pricing.setup.badge}</div>
              <div className="flex items-end gap-2 mb-2">
                <span className="text-5xl font-black text-foreground">{T.tvMenuBoards.pricing.setup.price}</span>
                <span className="text-muted-foreground font-light mb-1">{T.tvMenuBoards.pricing.setup.currency}</span>
              </div>
              <p className="text-muted-foreground text-sm font-light leading-relaxed">
                {T.tvMenuBoards.pricing.setup.description}
              </p>
            </motion.div>
            ...
            >
              <div className="absolute top-4 right-4">
                <span className="inline-flex items-center px-3 py-1 rounded-full bg-orange-500 text-white text-[10px] font-black tracking-[0.15em] uppercase">
                  {T.tvMenuBoards.pricing.monthly.periodLabel}
                </span>
              </div>
              <div className="text-xs font-black tracking-[0.2em] text-orange-500 uppercase mb-4">{T.tvMenuBoards.pricing.monthly.badge}</div>
              <div className="flex items-end gap-2 mb-2">
                <span className="text-5xl font-black text-foreground">{T.tvMenuBoards.pricing.monthly.price}</span>
                <span className="text-muted-foreground font-light mb-1">{T.tvMenuBoards.pricing.monthly.currency}</span>
              </div>
              <p className="text-muted-foreground text-sm font-light leading-relaxed">
                {T.tvMenuBoards.pricing.monthly.description}
              </p>
            </motion.div>
          </div>
        </div>
      ...
      <section className="relative border-t border-border overflow-hidden">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-20 sm:py-28">
          <div className="text-center mb-14">
            <div className="flex justify-center mb-6">
              <div className="px-4 py-1 rounded-full border border-orange-500/20 bg-orange-500/5 text-orange-500 text-[10px] sm:text-xs font-bold tracking-[0.2em] uppercase">
                {T.tvMenuBoards.features.pill}
              </div>
            </div>
            <h2 className="text-3xl sm:text-5xl font-serif italic mb-4">
              {T.tvMenuBoards.features.title}
              <br />
              <span className="text-gradient font-sans not-italic font-black">{T.tvMenuBoards.features.titleHighlight}</span>
            </h2>
          </div>
      ...
      <section className="relative border-t border-border bg-muted/30 overflow-hidden">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-20 sm:py-28">
          <div className="text-center mb-14">
            <div className="flex justify-center mb-6">
              <div className="px-4 py-1 rounded-full border border-orange-500/20 bg-orange-500/5 text-orange-500 text-[10px] sm:text-xs font-bold tracking-[0.2em] uppercase">
                {T.tvMenuBoards.whyUs.pill}
              </div>
            </div>
            <h2 className="text-3xl sm:text-5xl font-serif italic">
              {T.tvMenuBoards.whyUs.title}
              <br />
              <span className="text-gradient font-sans not-italic font-black">{T.tvMenuBoards.whyUs.titleHighlight}</span>
            </h2>
          </div>
      ...
      <section className="relative border-t border-border overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 py-24 sm:py-32 text-center">
          ...
          >
            {T.tvMenuBoards.cta.socialProof.map((item) => (
